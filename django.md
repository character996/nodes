...
建立路径  path函数
python manage.py migrate   检查INSTALLED_APPS设置，为需要建立数据库的应用建立数据库
### 定义模型和元数据
数据迁移 python manage.py sqlmigrate polls 可以输出数据库执行的操作set guifont=YaHei\ Consolas\ Hybrid\ 11.5
### 改变模型需要三步：
...
数据库查询优化   Teacher.objects.all().select_related('subject') 可以聚合查询 
...
前后端分离，前端进行页面展示，后端提供数据，前后端约定数据交互接口，前端通过HTTP请求来获取数据。后端主要提供JSON格式的数据，调用JsonResponse对象来将字典对象进行处理，封装为JSON返回数据。
